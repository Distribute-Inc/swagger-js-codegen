...
var getJSCode = function(opts, type) {
    ...
    if(type === 'custom') {
        ...
        if (type === 'es6') {
          method = fs.readFileSync(__dirname + '/../templates/es6-method.mustache', 'utf-8');
        } else {
    ...
    // NOTE(dmr, 2017-09-06): lint crashes codegen, complaining about
    // defining functions in a loop. Not knowing how to relax this
    // (strict=false doesnt do it), I simply removed the throw-loop
    // below, and if I'm removing the throw-loop there's no point in
    // doing the lint in the first place
    //
    // var lintOptions = {
...
var getTypeScriptDefinition = function(opts, type, resultTypeFn){
    ...
    // data.__methods = _.cloneDeep(data.methods);

    // data.__methods.forEach(function(method) {
    //     method.parameters = method.parameters.filter(function(param) {
    //       return param.name !== 'user-api-key';
    //     })
    // })

    // don't put user-api-key in method params, it will be automatically injected
    // where it is needed.
...
exports.CodeGen = {
    ...
    getNodeTypeScriptDefinition: function(opts){
        return getTypeScriptDefinition(opts, 'node', function (resultType) {
            return 'Q.Promise<' + resultType + '>';
        });
    },
    getTypeScriptDefinition: function(opts){
        opts.imports = [{name: 'Axios', path: 'axios'}];  // requires axios as a dependency.
        return getTypeScriptDefinition(opts, 'typescript', function (resultType) {
            return 'Axios.AxiosPromise<'+ resultType + '>';
        })
    },
    getAngularTypeScriptDefinition: function(opts){
        return getTypeScriptDefinition(opts, 'angular', function (resultType) {
